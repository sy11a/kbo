...
| assistant `tool_use` Write/Edit/NotebookEdit | `knowledge.written` (v2) | content fields stripped from `raw` and replaced by `<field>_size`; `contenthash` stays null like mined reads (ADR-0030); `linkcount` stays null — the note on disk may have moved on since the write, and harvest never re-reads subjects (BL-038) |
