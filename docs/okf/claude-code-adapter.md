...
- `task`: first match of the [registry](registry.md)'s optional `taskPattern` (env override `KBO_TASK_PATTERN`) in the cwd's git branch, read from `.git/HEAD` directly (no subprocess); `null` when no pattern is configured (ADR-0031). Raw branch in `data.branch` on `session.started`.
