...
| `knowledge.written` | Corpus | capture ROI ("notes written during captures ever read again?"); linking discipline (`linkcount`) | path, `contenthash`/`size` of the on-disk file (write→read correlation; written content itself is never embedded — ADR-0030), `linkcount` = distinct normalized wikilink targets, live hooks only (BL-038) | v2 (v1 lines stay valid as v1 — additive bump, ADR-0002) |
