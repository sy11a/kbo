...
- `kbo rebuild` bulk-loads events through DuckDB's Appender API instead of per-row `INSERT` commands: ~16s → ~1.6s on a 23.6k-event store, identical output. Closes the rebuild-throughput backlog item.
...
- Dashboard "Constitution fleet" panel (ADR-0038): legislated repos (scanned via their `docs/ai/manifest.json` — no fleet registry to maintain) against the current legislator constitution version, one row per repo, red when behind or unreadable. Opt-in via a top-level `constitution: {versionFile, scanRoots}` registry block (optional `exclude` skips named directories, e.g. archives); without it the panel is absent. The `kbo report` summary line gains `fleet: N repo(s), M behind vX`.
...
- Dead-man thresholds are cadence-aware (ADR-0037): weekly jobs (`report`, `audit`) are flagged silent after 9.5 days (6.5-day due point + the 3-day grace) instead of the flat 3 days that falsely alarmed `kbo doctor` and reddened two dashboard tiles from day 3 to day ~7 of every week. Daily jobs, capture-drop staleness, and agent last-seen tiles keep the 3-day rule.
